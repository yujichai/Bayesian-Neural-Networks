...
class Linear_2L(nn.Module):
    ...
    def sample_predict(self, x, Nsamples):
        ...
        predictions = x.data.new(1, x.shape[0], self.output_dim)

        return predictions


class Deterministic_net(BaseNet):
    eps = 1e-6
    ...
    def sample_eval(self, x, y, Nsamples=1, logits=True, train=False):
        x, y = to_variable(var=(x, y.long()), cuda=self.cuda)

        out = self.model.sample_predict(x, 1)
    ...
    def all_sample_eval(self, x, y, Nsamples=1):
        x, y = to_variable(var=(x, y.long()), cuda=self.cuda)
